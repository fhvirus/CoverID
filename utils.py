...
def match_one_song_features(features_list,
    ...
    for features, name in features_list: 
        ...
        song_chroma = chroma_features(song_samples, sr_song, hop_time=200, n_fft=2048, variation="norm")
        if transpose:
...
def match_all_songs_features(database: dict[str, pydub.AudioSegment],
    ...
    for name, data in tqdm(database.items()):
        ...
        sr = data.frame_rate
        features=chroma_features(data_samples, sr, hop_time=200, n_fft=2048, variation="norm")
        if transpose:
    ...
    for name, song in tqdm(covers.items()):
        ...
        if debug_mode and i < 3:  # Debug first 3 matches
            ...
            for db_name, db_audio in database.items():
                ...
                db_chroma = chroma_features(db_samples, db_sr, hop_time=200, n_fft=2048, variation="norm")
                if transpose:
...
def chroma_feature_by_beat(sample):
    ...
    res = []
    _, beats = librosa.beat.beat_track(y=y, sr=sample_rate, units='samples')
    for i in range(1, len(beats)):
        seg = np.array(y[beats[i - 1] : beats[i]])
        feat = librosa.feature.chroma_stft(y=seg, sr=sample_rate).sum(axis=1)
        res.append(feat / np.linalg.norm(feat))
...
def match_all_songs_features_beat(database: dict[str, pydub.AudioSegment],
    ...
    def get_chroma_feature(sample):
        sample = sample.set_channels(1) if sample.channels > 1 else sample
        return chroma_feature_by_beat(sample)
